...
type GroupCodeBusiness struct {
}

const maxContinuity = 4
const maxRepeat = 3

func (b *GroupCodeBusiness) RandomCode(IsSpecial bool) (int64, error) {
...
func (b *GroupCodeBusiness) RandomCodes(count int64, IsSpecial bool) ([]int64, error) {
	...
	var randoms []string
	if IsSpecial {
		randoms, err = r.RandomGroupSpecialCodes(count)
	} else {
		randoms, err = r.RandomGroupCodes(count)
	}
...
func (b *GroupCodeBusiness) Generate(digit int) error {
	r := cache.RedisServer{}
	if digit == 0 {
	...
	if _, err := r.GenerateGroupCodes(int64(digit), codes); err != nil {
		return err
	}
	if _, err := r.GenerateGroupSpecialCodes(int64(digit), specialCodes); err != nil {
		return err
	}
